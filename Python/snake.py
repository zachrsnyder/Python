...
def round_window(screenprop, multiple):
    return multiple * round(screenprop / multiple)

window = Tk()

SPACE_SIZE = 20
GAME_WIDTH = round_window((.5 * window.winfo_screenwidth()), SPACE_SIZE) 
GAME_HEIGHT = round_window((.5 * window.winfo_screenheight()), SPACE_SIZE)
SPEED = 70
BODY_PARTS = 3
SNAKE_COLOR = "#00FF00"
FOOD_COLOR = "#FF0000"
...
canvas.pack()
gameOver = Label(window, bg="black", fg="red", text=f"GAME OVER\n\nScore: {score}", font=('consolas', 60),height=GAME_HEIGHT, width=GAME_WIDTH)
